...
class ResponseCache:
    ...
    def get(self, query: str) -> Optional[str]:
        ...
        """
        embed_query = self.embedder.encode(query)

        for k,c in self._cache.items():
                
                dot=embed_query.dot(c["embedding"])

                if dot >= self.threshold:
                    # print(self.cache)
                    # return c["response"]

                    if time.time() - c["timestamp"] < self.ttl:
                        self._hits += 1
                        return c["response"]
                    else:
                        # Expired - remove it
...
# print(f'4. Lowercase lookup: {result}  (HIT - semantic same!)')
